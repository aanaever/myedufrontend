<template>
  <el-affix :offset="10" class="fixed-baby full-only">
    <el-card shadow="hover" class="details-card">
      ...
      <h1 v-if="singleCourse">{{ singleCourse.price }} руб.</h1>
      <h1 v-else>Price not available</h1>
      ...
      <div v-else>
        <p class="grey-sub">Вы уже приобрели курс</p>
        <el-button class="black btn" @click="goToCourse(singleCourse?.id)">
          Продолжить обучение <arrow-right class="myarrow" />
        </el-button>
      </div>
      ...
      <div>
        ...
        <ul style="padding-left: 20px">
          <li class="obj-item">Пожизненный доступ</li>
          <li class="obj-item">Видеолекции в хорошем качестве</li>
          <li class="obj-item">Доступ через мобильные устройства</li>
        </ul>
      </div>
    </el-card>
  </el-affix>
