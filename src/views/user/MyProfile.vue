<template>
  <h3 class="cart-header">Мой профиль</h3>
  <div class="main-view" style="height: 80vh" v-loading="isLoading">
    ...
    <div class="summary">
      ...
      <el-row :gutter="5" justify="space-around">
        <el-col v-for="item in summaryList" :key="item.title" :span="4">
          <div class="my-title">{{ getTranslation(item.title) }}</div>
    ...
    <div class="recently">
      <h3 class="serif-head">Ваши последние курсы</h3>
      <div class="recentBox" v-if="courseList.length > 0">
        ...
        <div @click="goToLearning()" class="recentSingle linky" v-if="courseList.length > 2">Посмотреть все</div>
      </div>
      <div v-else class="nodata">Нет информации</div>
    </div>

    <!-- EDIT PROFILE DIALOG -->
    <el-dialog v-model="dialogShow" title="Обновите свой профиль" width="40%">
...
const translations: Record<string, string> = {
  JOINED: 'Зарегестрирован ',
  COMPLETED: 'Завершено курсов',
  OWNING: 'Куленные курсы'
};
...
function updateProfile() {
  ...
  ProfileService.updateMine(userInfo)
    .then(res => {
      ...
      ElMessage.success("Ваш профиль был обновлен!");
    })
...
onMounted(() => {
  document.title = "Мой профиль | MyEdu";
  getProfileInfo();
  getUserProgress();
});
</script>
