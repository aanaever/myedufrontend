<template>
  <h3 class="cart-header">Корзина ({{ totalElements }})</h3>
  ...
  <div class="cart-content" v-loading="store.getIsLoggedIn && isLoading">
    <div class="main-view" :class="{ wider: cartItems.length === 0 }">
      ...
      <div v-if="!store.getCartCount" class="empty-view">
        <el-empty description="Ваша корзина пуста"></el-empty>

        <router-link to="/">
          <el-button plain class="btn purple" style="width: 15em"> Продолжите Ваши покупки </el-button>
        </router-link>
      </div>
        ...
        <el-row v-for="course in cartItems" :key="course.id">
          <el-space size="large" direction="vertical">
            <el-card class="w-card" shadow="hover">
              ...
              <el-icon class="w-delete" @click="removeCart(course.id)"><delete-filled /> <span>Удалить </span> </el-icon>
            </el-card>
          </el-space>
        </el-row>
